...
func TestCheckOnce_VirtualMachine(t *testing.T) {
	...
	origVirtEnv := currentVirtEnv
	defer func() { currentVirtEnv = origVirtEnv }()

	// Set up a KVM virtualization environment
...
func TestCheckOnce_EventCreation(t *testing.T) {
	...
	// Set up a non-KVM virtualization environment
	currentVirtEnv = host.VirtualizationEnvironment{
		Type:  "baremetal",
		IsKVM: false,
	}
...
func TestCheckOnce_ListFuncError(t *testing.T) {
	...
	c.listFunc = func(ctx context.Context) ([]pci.Device, error) {
		called = true
		return nil, testErr
	}
