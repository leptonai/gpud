...
type component struct {
	...
	connsCongestedPctMetricsStore     components_metrics.Store
	connsMaxBackgroundPctMetricsStore components_metrics.Store
}
...
func (c *component) CheckOnce() {
	...
	for _, info := range infos {
		...
		if err := c.setConnectionsCongestedPercent(c.ctx, info.DeviceName, info.CongestedPercent, now); err != nil {
			d.err = err
			return
		}
		if err := c.setConnectionsMaxBackgroundPercent(c.ctx, info.DeviceName, info.MaxBackgroundPercent, now); err != nil {
			d.err = err
			return
		}
		...
		if info.CongestedPercent > c.congestedPercentAgainstThreshold {
			msgs = append(msgs, fmt.Sprintf("congested percent against threshold %.2f exceeds threshold %.2f", info.CongestedPercent, c.congestedPercentAgainstThreshold))
		}
		if info.MaxBackgroundPercent > c.maxBackgroundPercentAgainstThreshold {
			msgs = append(msgs, fmt.Sprintf("max background percent against threshold %.2f exceeds threshold %.2f", info.MaxBackgroundPercent, c.maxBackgroundPercentAgainstThreshold))
		}
		if len(msgs) == 0 {
			continue
		}
