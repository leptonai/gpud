...
func TestCheckKubeletReadOnlyPortListening(t *testing.T) {
	...
	// Mock runtime.GOOS to test non-Linux behavior
	t.Run("non-linux OS", func(t *testing.T) {
		// Override GOOS to a non-linux value
		if origGOOS == "linux" {
		...
		result := CheckKubeletReadOnlyPortListening(ctx, DefaultKubeletReadOnlyPort)
		assert.False(t, result, "Should return false on non-linux OS")
	})
	...
	t.Run("healthy kubelet server", func(t *testing.T) {
		...
		// Since we can't easily mock the pkg_file.LocateExecutable function,
		// this test can't be fully comprehensive, but we can at least make sure
		// it successfully connects to our mock server
		result := CheckKubeletReadOnlyPortListening(ctx, int(port))
		if runtime.GOOS == "linux" {
			// On Linux, the function first checks if kubelet is in PATH
			// We can't guarantee the test result without mocking that check
			t.Logf("On Linux system, result is %v", result)
		} else {
			// On non-Linux systems, it should return false directly
...
// Test for connection error in checkOnce
func Test_componentCheckOnce_ConnectionError(t *testing.T) {
	// Create a closed server to force connection error
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}))
	portStr := srv.URL[len("http://127.0.0.1:"):]
	port, _ := strconv.ParseInt(portStr, 10, 32)
	srv.Close() // Close immediately to force connection error
	...
	// Create component with our closed port
	c := &component{
		ctx:                    ctx,
		cancel:                 cancel,
		kubeletReadOnlyPort:    int(port),
	...
	// We can't assert on KubeletPidFound as it depends on the test environment
	assert.Error(t, c.lastData.err)
	assert.True(t, c.lastData.connErr)
}
...
// Additional test for checkOnce handling context cancellation
func Test_componentCheckOnce_ContextCancellation(t *testing.T) {
	// Create a context that's already canceled
	ctx, cancel := context.WithCancel(context.Background())
	cancel() // Cancel immediately
	...
	// checkOnce should handle canceled context gracefully
	c.CheckOnce()
	...
	// Context canceled error should be detected
	assert.Error(t, c.lastData.err)
	assert.Contains(t, c.lastData.err.Error(), "context canceled")
}
