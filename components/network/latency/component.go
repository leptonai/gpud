...
type component struct {
	...
	measureLatencies func(context.Context) (latency.Latencies, error)

	lastMu   sync.RWMutex
	lastData *Data

	metricsMu                      sync.RWMutex
	edgeInMillisecondsMetricsStore components_metrics.Store
}
...
func New(ctx context.Context) components.Component {
	...
	return &component{
		...
		measureLatencies: func(ctx2 context.Context) (latency.Latencies, error) {
...
func (c *component) CheckOnce() {
	...
	var err error
	d.EgressLatencies, err = c.measureLatencies(cctx)
	if err != nil {
		d.err = err
		return
	}
	...
	for _, latency := range d.EgressLatencies {
		if err := c.setEdgeInMilliseconds(
			cctx,
