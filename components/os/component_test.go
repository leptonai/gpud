...
// TestEvaluateKernelPanicSuggestedActions tests the evaluateKernelPanicSuggestedActions function
func TestEvaluateKernelPanicSuggestedActions(t *testing.T) {
	now := time.Now()

	t.Run("no kernel panic events", func(t *testing.T) {
		cr := &checkResult{
			ts: now,
		}
		rebootEvents := eventstore.Events{}
		kernelPanicEvents := eventstore.Events{} // Empty

		evaluateKernelPanicSuggestedActions(cr, rebootEvents, kernelPanicEvents)

		// Should return without setting any suggested actions
	...
	t.Run("case 1 - no reboot events", func(t *testing.T) {
		...
		kernelPanicEvents := eventstore.Events{
			{
				Component: Name,
	...
	t.Run("case 2 - one reboot after kernel panic", func(t *testing.T) {
		cr := &checkResult{
			ts: now,
		}
		rebootEvents := eventstore.Events{
			{
				Time:    now.Add(-2 * time.Hour), // Reboot happened
				Name:    "reboot",
				Type:    string(apiv1.EventTypeWarning),
		...
		kernelPanicEvents := eventstore.Events{
			{
				Component: Name,
				Time:      now.Add(-3 * time.Hour), // First panic before reboot
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
			{
				Component: Name,
				Time:      now.Add(-1 * time.Hour), // Second panic after reboot
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
	...
	t.Run("case 3 - multiple kernel panic -> reboot sequences", func(t *testing.T) {
		cr := &checkResult{
			ts: now,
		}
		rebootEvents := eventstore.Events{
			...
			{
				Time:    now.Add(-2 * time.Hour), // Second reboot
				Name:    "reboot",
				Type:    string(apiv1.EventTypeWarning),
				Message: "system reboot detected",
			},
		...
		kernelPanicEvents := eventstore.Events{
			{
				Component: Name,
				Time:      now.Add(-6 * time.Hour), // First panic
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
			{
				Component: Name,
				Time:      now.Add(-4 * time.Hour), // Second panic after first reboot
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
			{
				Component: Name,
				Time:      now.Add(-1 * time.Hour), // Third panic after second reboot
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
	...
	t.Run("edge case - reboot but no subsequent panic", func(t *testing.T) {
		cr := &checkResult{
			ts: now,
		}
		rebootEvents := eventstore.Events{
			{
				Time:    now.Add(-1 * time.Hour), // Recent reboot
				Name:    "reboot",
				Type:    string(apiv1.EventTypeWarning),
				Message: "system reboot detected",
			},
		...
		kernelPanicEvents := eventstore.Events{
			{
				Component: Name,
				Time:      now.Add(-2 * time.Hour), // Panic before reboot
				Name:      eventNameKernelPanic,
				Type:      string(apiv1.EventTypeWarning),
				Message:   "kernel panic detected",
			},
...
// TestCheckKernelPanicWithRebootHistory tests the checkKernelPanicWithRebootHistory integration
func TestCheckKernelPanicWithRebootHistory(t *testing.T) {
	ctx := context.Background()
	now := time.Now()
	...
	t.Run("kernel panic with reboot after", func(t *testing.T) {
		mockBucket := &mockBucket{
			events: eventstore.Events{
				{
		...
		mockRebootStore := &mockRebootEventStore{
			events: eventstore.Events{
				{
					Time:    now.Add(-1 * time.Hour), // Reboot after panic
					Name:    pkghost.EventNameReboot,
					Type:    string(apiv1.EventTypeWarning),
		...
		// Should remain healthy (reboot after panic suggests recovery)
		assert.Equal(t, apiv1.HealthStateTypeHealthy, cr.health)
		assert.Equal(t, "ok", cr.reason)
		assert.Nil(t, cr.suggestedActions)
	})

	t.Run("recent kernel panic without reboot", func(t *testing.T) {
		mockBucket := &mockBucket{
			events: eventstore.Events{
				{
		...
		// Should be unhealthy with reboot suggestion
		assert.Equal(t, apiv1.HealthStateTypeUnhealthy, cr.health)
		assert.Equal(t, "kernel panic detected", cr.reason)
		assert.NotNil(t, cr.suggestedActions)
		assert.Len(t, cr.suggestedActions.RepairActions, 1)
		assert.Equal(t, apiv1.RepairActionTypeRebootSystem, cr.suggestedActions.RepairActions[0])
	})

	t.Run("multiple kernel panics with reboots", func(t *testing.T) {
		mockBucket := &mockBucket{
			events: eventstore.Events{
				{
		...
		// Should be unhealthy with hardware inspection suggestion
		assert.Equal(t, apiv1.HealthStateTypeUnhealthy, cr.health)
		assert.Equal(t, "kernel panic detected", cr.reason)
		assert.NotNil(t, cr.suggestedActions)
		assert.Len(t, cr.suggestedActions.RepairActions, 1)
