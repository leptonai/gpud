...
//		other commit: ccb3ce01b143ca5d39bf8eed68601d827d547718
//		go version: go1.23.5
func getTailscaledVersion() (string, error) {
	p, err := pkgfile.LocateExecutable("tailscaled")
	if err != nil {
		return "", err
	}
	out, err := exec.Command(p, "--version").CombinedOutput()
	if err != nil {
		return "", fmt.Errorf("tailscaled --version failed: %w: %s", err, strings.TrimSpace(string(out)))
	}
	return parseTailscaledVersion(string(out))
}
...
func parseTailscaledVersion(out string) (string, error) {
	return parseVersionFromOutput(out, "tailscaled")
}
