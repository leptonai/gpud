...
func recordGPMMetricByID(metricID nvml.GpmMetricId, gpuID string, pct float64) {
	...
	case nvml.GPM_METRIC_SM_OCCUPANCY:
		metricGPUSMOccupancyPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_INTEGER_UTIL:
		metricGPUIntUtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_ANY_TENSOR_UTIL:
		metricGPUAnyTensorUtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_DFMA_TENSOR_UTIL:
		metricGPUDFMATensorUtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_HMMA_TENSOR_UTIL:
		metricGPUHMMATensorUtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_IMMA_TENSOR_UTIL:
		metricGPUIMMATensorUtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_FP64_UTIL:
		metricGPUFp64UtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_FP32_UTIL:
		metricGPUFp32UtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)

	case nvml.GPM_METRIC_FP16_UTIL:
		metricGPUFp16UtilPercent.With(prometheus.Labels{pkgmetrics.MetricLabelNamePrefix + "uuid": gpuID}).Set(pct)
