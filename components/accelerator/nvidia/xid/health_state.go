...
func evolveHealthyState(events eventstore.Events, devices map[string]device.Device, rebootThreshold int) (ret apiv1.HealthState) {
	...
	if lastXidErr == nil {
		...
		reason = newXIDErrorReasonWithDetail(int(lastXidErr.Xid), lastXidErr.SubCode, lastXidErr.SubCodeDescription, lastXidErr.InvestigatoryHint, lastXidErr.DeviceUUID, devices)
	}
...
func newXIDErrorReasonWithDetail(xidVal, subCode int, subCodeDesc, investigatoryHint, deviceID string, devices map[string]device.Device) string {
	var suffix string
	...
	// Append investigatory hint to help differentiate errors with the same Unit but different root causes.
	// For example, two NETIR_LINK_EVT errors might have different hints like "[peer]" vs "[software]".
	if investigatoryHint != "" {
		codeLabel = fmt.Sprintf("%s [%s]", codeLabel, investigatoryHint)
	}
...
func enrichEventWithDetail(ev eventstore.Event, xidErr *xidErrorEventDetail, devices map[string]device.Device) eventstore.Event {
	...
	ev.Message = newXIDErrorReasonWithDetail(int(xidErr.Xid), xidErr.SubCode, xidErr.SubCodeDescription, xidErr.InvestigatoryHint, xidErr.DeviceUUID, devices)
