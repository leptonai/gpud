...
func TestRegexNVSwitchFatalSXidFromLog(t *testing.T) {
	...
	for _, test := range tests {
		matched, err := regexp.MatchString(regexNVSwitchFatalSXidFromLog, test.log)
		if err != nil {
...
func TestRegexNVSwitchNonFatalSXidFromLog(t *testing.T) {
	...
	for _, test := range tests {
		matched, err := regexp.MatchString(regexNVSwitchNonFatalSXidFromLog, test.log)
		if err != nil {
			t.Fatalf("Error matching regex: %v", err)
		}
		if matched != test.matches {
...
func TestRegexNVSwitchNVLinkFailureFromLog(t *testing.T) {
	...
	for _, test := range tests {
		matched, err := regexp.MatchString(regexNVSwitchNVLinkFailureFromLog, test.log)
		if err != nil {
			t.Fatalf("Error matching regex: %v", err)
		}
		if matched != test.matches {
