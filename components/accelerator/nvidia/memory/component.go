...
import (
	...
	nvidia_query_metrics_memory "github.com/leptonai/gpud/components/accelerator/nvidia/query/metrics/memory"
...
func (c *component) Metrics(ctx context.Context, since time.Time) ([]components.Metric, error) {
	...
	totalBytes, err := nvidia_query_metrics_memory.ReadTotalBytes(ctx, since)
	if err != nil {
		return nil, fmt.Errorf("failed to read total bytes: %w", err)
	}
	usedBytes, err := nvidia_query_metrics_memory.ReadUsedBytes(ctx, since)
	if err != nil {
		return nil, fmt.Errorf("failed to read used bytes: %w", err)
	}
	usedPercents, err := nvidia_query_metrics_memory.ReadUsedPercents(ctx, since)
	if err != nil {
...
func (c *component) RegisterCollectors(reg *prometheus.Registry, dbRW *sql.DB, dbRO *sql.DB, tableName string) error {
	c.gatherer = reg
	return nvidia_query_metrics_memory.Register(reg, dbRW, dbRO, tableName)
}
