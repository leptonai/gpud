...
func TestDropStickyWindow(t *testing.T) {
	...
	mockStore.events = []infinibandstore.Event{
		{
			Time:        dropTime,
			Port:        infiniband.IBPort{Device: "mlx5_0", Port: uint(1)},
	...
	mockStore.events = []infinibandstore.Event{
		{
			...
			Port:        infiniband.IBPort{Device: "mlx5_7", Port: uint(1)},
			EventType:   infinibandstore.EventTypeIbPortDrop,
	...
	mockStore.events = []infinibandstore.Event{
		{
			...
			Port:        infiniband.IBPort{Device: "mlx5_1", Port: uint(1)},
...
func TestDropStickyWindowEdgeCases(t *testing.T) {
	...
	mockStore.events = []infinibandstore.Event{
		{
			Time:        oldDropTime,
			Port:        infiniband.IBPort{Device: "mlx5_8", Port: uint(1)}, // Dormant port
			EventType:   infinibandstore.EventTypeIbPortDrop,
			EventReason: "mlx5_8 port 1 down since " + oldDropTime.Format(time.RFC3339),
		},
		{
			Time:        oldDropTime,
			Port:        infiniband.IBPort{Device: "mlx5_9", Port: uint(1)}, // Another dormant port
			EventType:   infinibandstore.EventTypeIbPortDrop,
...
func (m *mockIBPortsStoreForStickyDrop) Insert(time.Time, []infiniband.IBPort) error {
	return nil
}
