...
func TestEvaluate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantReason:  "not enough LinkUp ports -- only 1 LinkUp ports found; some ports must be missing",
			wantHealthy: false,
			wantErr:     false,
		},
		{
			...
			wantReason:  "not enough LinkUp ports -- only 0 LinkUp ports found; some ports must be missing",
			wantHealthy: false,
			wantErr:     false,
		},
		{
			...
			wantReason:  "not enough LinkUp ports -- only 0 LinkUp ports found; some ports might be down or polling, 2 'Disabled' or 'Polling' devices with Rate > 200 found (mlx5_0, mlx5_1)",
			wantHealthy: false,
			wantErr:     false,
		},
	}
...
func TestEvaluateWithTestData(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantReason:  "not enough LinkUp ports -- only 8 LinkUp ports found; some ports must be missing",
			wantHealthy: false,
			wantErr:     false,
		},
	}
