...
func TestRunPurgeWithContext(t *testing.T) {
	...
	store := &ibPortsStore{
		...
		ibPortDropThreshold: 1 * time.Minute,
	}
...
func TestRunPurgeWithOldTombstoneTimestamp(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	store := &ibPortsStore{
		...
		minInsertInterval:   0,
		purgeInterval:       1 * time.Millisecond,
		ibPortDropThreshold: 10 * time.Minute,
	}
	...
	// Set an old tombstone timestamp to trigger the special logic
	oldTombstoneTime := time.Now().Add(-2 * time.Hour)
	err = setTombstoneTimestamp(ctx, dbRW, store.metadataTable, oldTombstoneTime)
	require.NoError(t, err)
	...
	// This will test the special case where tombstone timestamp affects purge behavior
	store.runPurge()
}
...
func TestGetPurgeBeforeTimestamp(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	require.False(t, purgeBefore.IsZero())
	require.False(t, purgeAllEvents)

	// Check that the timestamp is approximately retention period ago
	expectedTime := s.getTimeNow().Add(-s.retentionPeriod)
	timeDiff := purgeBefore.Sub(expectedTime)
	assert.True(t, timeDiff >= -time.Second && timeDiff <= time.Second,
		"Expected purgeBefore to be within 1 second of retention period ago")
}

func TestGetPurgeBeforeTimestampWithTombstone(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	// Should use tombstone timestamp and purge all events
	purgeBefore, purgeAllEvents := s.getPurgeBeforeTimestamp()
	require.False(t, purgeBefore.IsZero())
	require.True(t, purgeAllEvents)

	// Check that the timestamp matches tombstone
	assert.Equal(t, tombstoneTime.Unix(), purgeBefore.Unix())
}

func TestGetPurgeBeforeTimestampWithOldTombstone(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	// Should use retention period instead of tombstone
	purgeBefore, purgeAllEvents := s.getPurgeBeforeTimestamp()
	require.False(t, purgeBefore.IsZero())
	require.False(t, purgeAllEvents)
...
func TestGetPurgeBeforeTimestampWithTombstoneError(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	// Should return zero time and false when getTombstoneTimestamp fails
	purgeBefore, purgeAllEvents := s.getPurgeBeforeTimestamp()
	require.True(t, purgeBefore.IsZero())
	require.False(t, purgeAllEvents)
}

func TestGetPurgeBeforeTimestampWithContextTimeout(t *testing.T) {
	ctx := context.Background()
	dbRW, dbRO, cleanup := sqlite.OpenTestDB(t)
	defer cleanup()
	...
	// Should return zero time and false when context times out
	purgeBefore, purgeAllEvents := s.getPurgeBeforeTimestamp()
	require.True(t, purgeBefore.IsZero())
	require.False(t, purgeAllEvents)
}
