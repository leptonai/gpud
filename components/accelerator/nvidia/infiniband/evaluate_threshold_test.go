...
func TestEvaluateIBSwitchFault(t *testing.T) {
	tests := []struct {
		name           string
		currentPorts   []infiniband.IBPort
		unhealthyPorts []infiniband.IBPort
		expectFault    bool
	}{
		{
			name:           "no ports",
		...
		{
			name: "all ports down - switch fault",
			currentPorts: []infiniband.IBPort{
				{Device: "mlx5_0", Port: 1, State: "Down", LinkLayer: "Infiniband"},
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
			},
			unhealthyPorts: []infiniband.IBPort{
				{Device: "mlx5_0", Port: 1, State: "Down", LinkLayer: "Infiniband"},
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
		...
		{
			name: "partial ports down - no switch fault",
			currentPorts: []infiniband.IBPort{
				{Device: "mlx5_0", Port: 1, State: "Active", LinkLayer: "Infiniband"},
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
			},
			unhealthyPorts: []infiniband.IBPort{
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
		...
		{
			name: "ethernet ports ignored",
			currentPorts: []infiniband.IBPort{
				{Device: "mlx5_0", Port: 1, State: "Down", LinkLayer: "Ethernet"},
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
			},
			unhealthyPorts: []infiniband.IBPort{
				{Device: "mlx5_1", Port: 1, State: "Down", LinkLayer: "Infiniband"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cr := &checkResult{
				unhealthyIBPorts: tt.unhealthyPorts,
			}

			evaluateIBSwitchFault(tt.currentPorts, cr)

			if tt.expectFault {
				assert.Equal(t, "ib switch fault, all ports down", cr.reasonIbSwitchFault)
			} else {
				assert.Empty(t, cr.reasonIbSwitchFault)
			}
		})
	}
}

// TestEvaluateIBPortsDrop tests the evaluateIBPortsDrop function
func TestEvaluateIBPortsDrop(t *testing.T) {
	now := time.Now()

	tests := []struct {
		name        string
		snapshots   ibPortsSnapshots
		...
		{
			name: "port down less than threshold",
			snapshots: ibPortsSnapshots{
				{
		...
		{
			name: "port down exceeds threshold",
			snapshots: ibPortsSnapshots{
				{
			...
			checkTime:   now,
			expectDrop:  true,
			dropDevices: []string{"mlx5_0"},
		},
		{
			name: "port recovered - no drop",
			snapshots: ibPortsSnapshots{
				{
					ts: now.Add(-6 * time.Minute),
					unhealthy: []infiniband.IBPort{
		...
		},
		{
			...
			checkTime:   now,
			expectDrop:  true,
			dropDevices: []string{"mlx5_0", "mlx5_1"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			evaluateIBPortsDrop(tt.snapshots, tt.threshold, cr)

			if tt.expectDrop {
				assert.NotEmpty(t, cr.reasonIbPortsDrop)
				for _, device := range tt.dropDevices {
					assert.Contains(t, cr.reasonIbPortsDrop, device)
				}
			} else {
				assert.Empty(t, cr.reasonIbPortsDrop)
			}
		})
	}
}
...
func TestEvaluateIBPortFlap(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:       "no snapshots",
			snapshots:  ibPortsSnapshots{},
			evalPeriod: 5 * time.Minute,
			checkTime:  now,
			expectFlap: false,
		},
		{
			name: "port flapped - down to active",
			snapshots: ibPortsSnapshots{
				{
					ts: now.Add(-4 * time.Minute),
					all: []infiniband.IBPort{
						{Device: "mlx5_0", Port: 1, State: "Down", LinkLayer: "Infiniband", TotalLinkDowned: 5},
					},
				},
				{
					ts: now.Add(-2 * time.Minute),
					all: []infiniband.IBPort{
						{Device: "mlx5_0", Port: 1, State: "Active", LinkLayer: "Infiniband", TotalLinkDowned: 6},
					},
				},
			},
			evalPeriod:  5 * time.Minute,
			checkTime:   now,
			expectFlap:  true,
			flapDevices: []string{"mlx5_0"},
		},
		{
			name: "port stayed down - no flap",
			snapshots: ibPortsSnapshots{
				{
					ts: now.Add(-4 * time.Minute),
					all: []infiniband.IBPort{
		...
		{
			name: "port stayed active - no flap",
			snapshots: ibPortsSnapshots{
				{
					ts: now.Add(-4 * time.Minute),
					all: []infiniband.IBPort{
						{Device: "mlx5_0", Port: 1, State: "Active", LinkLayer: "Infiniband", TotalLinkDowned: 5},
					},
				},
				{
					ts: now.Add(-2 * time.Minute),
		...
		{
			name: "flap outside eval period - ignored",
			snapshots: ibPortsSnapshots{
				{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			evaluateIBPortFlap(tt.snapshots, tt.evalPeriod, cr)

			if tt.expectFlap {
				assert.NotEmpty(t, cr.reasonIbPortsFlap)
				for _, device := range tt.flapDevices {
					assert.Contains(t, cr.reasonIbPortsFlap, device)
				}
			} else {
				assert.Empty(t, cr.reasonIbPortsFlap)
			}
		})
	}
}
