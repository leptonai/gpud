	...
	mockStore := &mockIBPortsStoreComprehensive{
		events: []infinibandstore.Event{
			// Very old drop (dormant port - 3 days old)
			{
				...
				Port:        infiniband.IBPort{Device: "mlx5_8", Port: uint(1)},
				EventType:   infinibandstore.EventTypeIbPortDrop,
			...
			// Old drop (dormant port - 2 hours old)
			{
				...
				Port:        infiniband.IBPort{Device: "mlx5_9", Port: uint(1)},
				EventType:   infinibandstore.EventTypeIbPortDrop,
...
func (m *mockIBPortsStoreComprehensive) Insert(time.Time, []infiniband.IBPort) error {
	return nil
}
