...
func TestPoller_processItem(t *testing.T) {
	now := time.Now()
...
func TestPollerStartStop(t *testing.T) {
	...
	q := &pollerImpl{
		...
		cfg:       poller_config.Config{QueueSize: 3},
		lastItems: []Item{},
	...
	q.Start(ctx, poller_config.Config{Interval: metav1.Duration{Duration: time.Second}}, "test1")
	q.Start(ctx, poller_config.Config{Interval: metav1.Duration{Duration: time.Second}}, "test2")
	q.Start(ctx, poller_config.Config{Interval: metav1.Duration{Duration: time.Second}}, "test3")
