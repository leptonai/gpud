...
func Command(cliContext *cli.Context) error {
	...
	log.Logger.Debugw("reading control plane login success")
	loginSuccess, err := pkgmetadata.ReadMetadata(rootCtx, dbRO, pkgmetadata.MetadataKeyControlPlaneLoginSuccess)
	if err != nil {
		return fmt.Errorf("failed to read control plane login success: %w", err)
	}
	log.Logger.Debugw("successfully read control plane login success")

	if loginSuccess == "" {
		ts, err := strconv.ParseInt(loginSuccess, 10, 64)
		if err != nil {
			return fmt.Errorf("failed to parse control plane login success: %w", err)
		}
		loginTimeUTC := time.Unix(ts, 0)
		nowUTC := time.Now().UTC()
		loginTimeHumanized := humanize.RelTime(loginTimeUTC, nowUTC, "ago", "from now")
		fmt.Printf("%s control plane login success at %s (machine id: %s)\n", cmdcommon.CheckMark, loginTimeHumanized, machineID)
	} else {
		fmt.Printf("%s control plane login information not found\n", cmdcommon.CheckMark)
	}

	log.Logger.Debugw("reading control plane join success")
	joinSuccess, err := pkgmetadata.ReadMetadata(rootCtx, dbRO, pkgmetadata.MetadataKeyControlPlaneJoinSuccess)
	if err != nil {
		return fmt.Errorf("failed to read control plane join success: %w", err)
	}
	log.Logger.Debugw("successfully read control plane join success")

	if joinSuccess == "" {
		ts, err := strconv.ParseInt(joinSuccess, 10, 64)
		if err != nil {
			return fmt.Errorf("failed to parse control plane join success: %w", err)
		}
		joinTimeUTC := time.Unix(ts, 0)
		nowUTC := time.Now().UTC()
		joinTimeHumanized := humanize.RelTime(joinTimeUTC, nowUTC, "ago", "from now")
		fmt.Printf("%s control plane join success at %s (machine id: %s)\n", cmdcommon.CheckMark, joinTimeHumanized, machineID)
	} else {
		fmt.Printf("%s control plane join information not found\n", cmdcommon.CheckMark)
	}
