...
func (s *Session) startReader(readerExit chan any) {
	...
	*lastPackageTimestamp = time.Now()
	...
	go s.handleReaderPipe(resp.Body, lastPackageTimestamp, goroutineCloseCh)
	for {
		var content Body
...
func (s *Session) handleReaderPipe(respBody io.ReadCloser, lastPackageTimestamp *time.Time, closec chan any) {
