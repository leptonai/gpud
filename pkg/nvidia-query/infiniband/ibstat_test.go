...
func TestEvaluatePortsAndRateWithIBStatCards(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			atLeastPorts:         2,
			atLeastRate:          200,
			wantErr:              errors.New("only 0 port(s) are active and >=200 Gb/s, expect >=2 port(s); 2 device(s) physical state Disabled (mlx5_0, mlx5_1)"),
		...
		},
		{
			...
			atLeastPorts:         4,
			atLeastRate:          200,
			wantErr:              errors.New("only 2 port(s) are active and >=200 Gb/s, expect >=4 port(s); 2 device(s) physical state Disabled (mlx5_1, mlx5_3)"),
			wantProblematicPorts: 2,
		},
		{
			name: "wrong rate",
		...
		},
		{
			...
			atLeastPorts:         3,
			atLeastRate:          200,
			wantErr:              errors.New("only 2 port(s) are active and >=200 Gb/s, expect >=3 port(s); 1 device(s) physical state Disabled (mlx5_1)"),
		...
		},
		{
			...
			atLeastPorts:         3,
			atLeastRate:          200,
			wantErr:              errors.New("only 2 port(s) are active and >=200 Gb/s, expect >=3 port(s); 1 device(s) physical state Disabled (mlx5_1); 1 device(s) physical state Polling (mlx5_3) -- connecton lost from this card to other cards/switches"),
			wantProblematicPorts: 2,
		},
		{
...
func TestIBStatCardsIBPorts(t *testing.T) {
	...
	require.Equal(t, 200, ports[0].RateGBSec)
