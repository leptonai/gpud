...
func (s *ibPortsStore) Insert(eventTime time.Time, ibPorts []infiniband.IBPort) error {
	...
	s.configMu.RLock()
	minInsertInterval := s.minInsertInterval
	historyTable := s.historyTable
	rootCtx := s.rootCtx
	s.configMu.RUnlock()

	if !lastTimestamp.IsZero() && eventTime.Before(lastTimestamp.Add(minInsertInterval)) {
		log.Logger.Warnw("skipping insert", "table", historyTable, "eventTime", eventTime, "lastTs", lastTimestamp, "minInsertInterval", minInsertInterval)
		return nil
	}
	...
	log.Logger.Infow("successfully inserted ib ports", "table", historyTable, "eventTime", eventTime, "ibPorts", len(ibPorts))
	return nil
}
