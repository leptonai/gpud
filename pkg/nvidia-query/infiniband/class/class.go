...
func loadSysFS(mountPoint string) (sysFS, error) {
	info, err := os.Stat(mountPoint)
	if err != nil {
		return sysFS{}, fmt.Errorf("could not read %q: %w", mountPoint, err)
	}
	if !info.IsDir() {
...
func (fs *sysFS) readDir(dir string) ([]fs.DirEntry, error) {
	return os.ReadDir(filepath.Join(fs.mountPoint, dir))
}
...
// /sys/class/infiniband.
func (fs sysFS) InfiniBandClass() (InfiniBandClass, error) {
	dirs, err := os.ReadDir(fs.mountPoint)
	if err != nil {
		return nil, err
	}
...
func (fs sysFS) parseInfiniBandDevice(name string) (*InfiniBandDevice, error) {
	device := InfiniBandDevice{Name: name}

	// fw_ver is exposed by all InfiniBand drivers since kernel version 4.10.
	value, err := fs.readFile(filepath.Join(name, "fw_ver"))
	if err != nil {
	...
	for _, f := range [...]string{"board_id", "hca_type"} {
		value, err := fs.readFile(filepath.Join(name, f))
		if err != nil {
	...
	portsDir := filepath.Join(name, "ports")
	ports, err := fs.readDir(portsDir)
	if err != nil {
		return nil, fmt.Errorf("failed to list InfiniBand ports at %q: %w", portsDir, err)
	}
...
func (fs sysFS) parseInfiniBandPort(portName string, port string) (*InfiniBandPort, error) {
	portNumber, err := strconv.ParseUint(port, 10, 32)
	if err != nil {
		return nil, fmt.Errorf("failed to convert %s into uint", port)
	}
	ibp := InfiniBandPort{Name: portName, Port: uint(portNumber)}

	// e.g., /sys/class/infiniband/mlx5_0/ports/1
	portDir := filepath.Join(portName, "ports", port)

	content, err := fs.readFile(filepath.Join(portDir, "state"))
	if err != nil {
		return nil, err
	}
	id, portName, err := parseState(content)
	if err != nil {
	...
	content, err = fs.readFile(filepath.Join(portDir, "phys_state"))
	if err != nil {
		return nil, err
	}
	id, portName, err = parseState(content)
	...
	ibp.PhysState = portName
	ibp.PhysStateID = id

	content, err = fs.readFile(filepath.Join(portDir, "rate"))
	if err != nil {
		return nil, err
	}
	ibp.Rate, err = parseRate(content)
	if err != nil {
	...
	if _, err := os.Stat(countersDir); err == nil {
		counters, err := fs.parseInfiniBandCounters(portDir)
		if err != nil {
			return nil, err
		}
	...
	if _, err := os.Stat(hwCountersDir); err == nil {
		hwCounters, err := fs.parseInfiniBandHwCounters(portDir)
		if err != nil {
			return nil, err
		}
...
func (fs sysFS) parseInfiniBandCounters(portDir string) (*InfiniBandCounters, error) {
...
func (fs sysFS) parseInfiniBandHwCounters(portDir string) (*InfiniBandHwCounters, error) {
