...
func (s *healthStateSetter) SetHealthy(healthy bool)     { s.setHealthy(healthy) }
func (s *healthStateSetter) SetUnhealthy(unhealthy bool) { s.setUnhealthy(unhealthy) }
func (s *healthStateSetter) SetDegraded(degraded bool)   { s.setDegraded(degraded) }
...
func RegisterHealthStateMetrics(componentName string) (HealthStateSetter, error) {
	...
	setter, err := RegisterHealthStateMetricsWithRegistere(DefaultRegisterer(), componentName)
	if err != nil {
		return nil, err
	}
...
// RegisterHealthStateMetricsWithRegistere registers the health state metrics for the given component.
// Returns the function to update the health state metrics.
func RegisterHealthStateMetricsWithRegistere(reg prometheus.Registerer, componentName string) (HealthStateSetter, error) {
