...
func TestParseJournalctlTimeShortISOWithError(t *testing.T) {
	tests := []struct {
		name     string
		line     []byte
		want     time.Time
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("ParseJournalctlTimeShortISO() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !got.Equal(tt.want) {
				t.Errorf("ParseJournalctlTimeShortISO() got = %v, want %v", got, tt.want)
			}
			if !bytes.Equal(line, tt.wantLine) {
				t.Errorf("ParseJournalctlTimeShortISO() line = %v, want %v", string(line), string(tt.wantLine))
			}
		})
	}
}
