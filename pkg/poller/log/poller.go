...
import (
	...
	poller_log_tail "github.com/leptonai/gpud/pkg/poller/log/tail"
...
type Poller interface {
	...
	// It returns all events by the default filters in the configuration.
	// Returns `github.com/leptonai/gpud/pkg/poller.ErrNoData` if there is no event found.
...
// It is converted from the underlying poller.
type Item struct {
...
func New(ctx context.Context, cfg poller_log_config.Config) (Poller, error) {
	return newPoller(ctx, cfg)
}

func newPoller(ctx context.Context, cfg poller_log_config.Config) (*pollerImpl, error) {
	...
	options := []poller_log_tail.OpOption{
		...
		poller_log_tail.WithExtractTime(cfg.ExtractTime),
		poller_log_tail.WithProcessMatched(cfg.ProcessMatched),
...
// This only catches the realtime/latest and all the future logs.
// Returns `github.com/leptonai/gpud/pkg/poller.ErrNoData` if there is no event found.
func (pl *pollerImpl) Find(since time.Time) ([]Item, error) {
	...
	for _, item := range polledItems {
		...
		for _, item := range itemsFromPollerOutput {
			...
			for _, f := range pl.cfg.SelectFilters {
