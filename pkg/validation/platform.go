...
// MinimumResourceCheck retrieves the platform resource information required to
// confirm that the host meets the minimum thresholds for joining the Lepton
// platform.
func MinimumResourceCheck(ctx context.Context) (PlatformRequirements, error) {
...
// MeetsMinimum returns true when the platform satisfies the minimum CPU core
// and memory requirements.
func (p PlatformRequirements) MeetsMinimum() bool {
	return p.LogicalCPUCores >= p.MinimumCPUCores &&
		p.TotalMemoryBytes >= p.MinimumMemory
}
