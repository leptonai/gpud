...
func GetProvider(publicIP string) *providers.Info {
	...
	if publicIP != "" {
		...
		log.Logger.Debugw("ASN lookup result", "asnResult", asnResult, "asnName", asnResult.AsnName)
		normalizedProvider := asn.NormalizeASNName(asnResult.AsnName)
		log.Logger.Debugw("normalized provider name", "normalizedProvider", normalizedProvider)
