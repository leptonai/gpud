...
// Custom HTTP client for testing that doesn't do HTTPS
var testHTTPClient = &http.Client{}

// Override the HTTP Post function for testing
func testPost(url, contentType string, body io.Reader) (*http.Response, error) {
	return testHTTPClient.Post(url, contentType, body)
}
...
func TestLogin_InvalidToken(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status": "invalid workspace token", "error": "unauthorized"}`))
	}))
	defer server.Close()
...
func TestLogin_ServerError(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status": "server error", "error": "internal error"}`))
	}))
	defer server.Close()
...
func TestLogin_InvalidJsonResponse(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`{"status": "error", "error": "invalid request`)) // Missing closing quote and brace
	}))
	defer server.Close()
...
// Create a mock implementation for testing Login with PublicIP error
type mockNetUtil struct{}

func (m *mockNetUtil) PublicIP() (string, error) {
	return "", fmt.Errorf("failed to get IP")
}

// TestLogin_WithPublicIPMock creates a custom HTTP client that mocks the complete Login with PublicIP flow
func TestLogin_WithPublicIPMock(t *testing.T) {
	// Testing a codepath that uses Public IP requires deeper mocking
	// For now, we'll focus on testing the login function directly since its functionality is the core
	t.Skip("Skipping test that requires mocking PublicIP function")
}
